...
export class MarketplaceComponent {
	private readonly _items: Item[] = [
		{
			...
			description: 'Soft knitted sweater perfect for autumn. Slightly oversized fit.',
			price: 15,
			images: [
...

		{
			...
			description: 'High-waisted denim with a relaxed 90s fit. Slight fading around knee.',
			price: 22,
	...
	public filteredOptions = computed<Item[]>(() =>
		this._items.filter((item) => item.title.toLowerCase().includes(this.search().toLocaleLowerCase())),
	);
}
